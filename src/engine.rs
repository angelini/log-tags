...
impl Engine {
    ...
    fn take(&mut self, plan: &Plan, count: usize) -> Result<Vec<String>> {
        ...
        'outer: for batch_interval in ReadIntervals::new(count, MAX_BATCH_SIZE) {
            for id in &plan.steps {
                match id {
                    ...
                    Id::Distinct(distinct_id) => {
                        println!("distinct_id: {:?}", distinct_id);
        ...
        println!("plan.filter_ids(): {:?}", plan.filter_ids());
        for filter_id in plan.filter_ids() {
            match combined_filter {
                ...
                None => {
                    println!("self.read_filter(filter_id): {:?}", self.read_filter(filter_id));
                    combined_filter = Some(self.read_filter(filter_id).clone())
                }
            }
        }
        println!("plan.distinct_ids(): {:?}", plan.distinct_ids());
        for distinct_id in plan.distinct_ids() {
            match combined_filter {
                Some(ref mut filter) => filter.intersect_with(self.read_distinct(distinct_id)),
                None => {
                    println!("self.read_distinct(distinct_id): {:?}", self.read_distinct(distinct_id));
                    combined_filter = Some(self.read_distinct(distinct_id).clone())
                }
            }
        }
